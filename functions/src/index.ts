...
app.get('**', (req, res) => {
  renderModuleFactory(AppServerModuleNgFactory, {
    ...
    .catch(err => console.log("couldn't fulfill request", err));
});
...
exports.onUpdateArticleDetail = functions.firestore
...
exports.trackCommentVotes = functions.database
...
exports.bubbleUpCommentCount = functions.database
  ...
  .onUpdate(async (change, context) => {
    ...
    const incrementCommentCount = (
      articleDocRef: admin.firestore.DocumentReference,
    ) => {
      return adminFS
    ...
    const parentCommentRef = adminDB.ref(
      `commentData/comments/${context.params.commentKey}`,
    );
...
exports.trackCommentAuthorsAndParents = functions.database
  ...
  .onCreate((snap, context) => {
    ...
    return Promise.all([
      commentRef.parent.parent
        .child(`commentsByParent/${comment.parentKey}/${key}`)
        .set(true),
      commentRef.parent.parent
        .child(`commentsByAuthor/${comment.authorId}/${key}`)
        .set(true),
    ]);
  });

//  trackUploadFiles entry point:
exports.trackFileUploads = functions.storage
  .object()
...
async function createCoverImageThumbnail(
...
exports.createHistoryObject = functions.firestore
  .document('articleData/articles/articles/{articleId}')
  .onWrite((change, context) => {
    if (context.eventType !== 'google.firestore.document.delete') {
      ...
      const historyRef = adminFS.doc(
        `articleData/articles/articles/${articleId}/history/${historyId}`,
      );
...
exports.createPreviewObject = functions.firestore
  .document('articleData/articles/articles/{articleId}')
  .onWrite((change, context) => {
    const articleObject = change.after.data();
